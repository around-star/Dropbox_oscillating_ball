...
class ODEBlock(nn.Module):
    ...
    """
    def __init__(self, device, odefunc, method, 
			odeint_rtol = 1e-4, odeint_atol = 1e-5, is_conv=False, adjoint=False):
    ...
    def forward(self, x, eval_times=None):
        ...
        if self.odefunc.augment_dim > 0:
            if self.is_conv:
                ...
                # aug = torch.zeros(x.shape[0], self.odefunc.augment_dim, x.shape).to(self.device)
        ...
        if self.adjoint:
            ...
            out = odeint(self.odefunc, x_aug, integration_time,
                         rtol=self.odeint_rtol, atol=self.odeint_atol, method='dopri5',
                         options={'max_num_steps': MAX_NUM_STEPS, 'step_size':0.05})
            # out = odeint(self.odefunc, x_aug, integration_time,
            #              rtol=self.odeint_rtol, atol=self.odeint_atol, method='euler')
