...
def create_LatentODE_model(input_dim, z0_prior, obsrv_std, device, 
    ...
    if poisson:
        ...
        latents_new = latents + 512 + vel_dim
        # latents_new = latents + 512 + latents
        linear = None
        ...
        ode_func_net = utils.create_net(dim, latents_new, 
            n_layers = gen_layers, n_units = units, nonlinear = nn.Tanh)
        # ode_func_net = utils.create_net(latents, latents, 
        #     n_layers = gen_layers, n_units = units, nonlinear = nn.Tanh)
    ...
    if z0_encoder == "mlp":
        ...
        concat_net = None
        # W = 256
        # layers = [nn.Linear(latents_new, W)]

        # layers += [layer(W, W)]
        
        # layers += [layer(W, latents)]        

        # concat_net = nn.ModuleList(layers)
    ...
    if z0_encoder == 'vae':
        ...
        decoder = Decoder(latents_new, 512, single_layer=True).to(device)
        # decoder = Decoder(latents_new, 512).to(device)
        # decoder = Decoder(512, 512, single_layer=True).to(device)
        # decoder = Decoder(latents, 512).to(device)
    ...
    # decoder_vel = Decoder(latents, vel_dim).to(device)

    # diffeq_solver = DiffeqSolver(gen_data_dim, gen_ode_func, 'midpoint', latents_new, 
    #     odeint_rtol = 1e-3, odeint_atol = 1e-4, device = device)
    # diffeq_solver = DiffeqSolver(gen_data_dim, gen_ode_func, 'bosh3', latents_new, 
    #    odeint_rtol = 1e-3, odeint_atol = 1e-4, device = device)
    ...
    # diffeq_solver = ODEBlock(device=device, odefunc=gen_ode_func, method='dopri5', 
