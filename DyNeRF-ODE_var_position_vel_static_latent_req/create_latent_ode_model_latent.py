...
from latent_ode_latent_loc import LatentODE
...
def create_LatentODE_model(input_dim, z0_prior, obsrv_std, device, 
    classif_per_tp = False, n_labels = 1, latents=8, units=100, gen_layers=3,
    ...
    if poisson:
        ...
        ode_func_net = utils.create_network_with_sum(dim, latents_new, 
            n_layers = gen_layers, n_units = units, nonlinear = nn.Tanh)
        
        # ode_func_net = utils.create_net(dim, latents_new, 
        #     n_layers = gen_layers, n_units = units, nonlinear = nn.ReLU)

        # ode_func_net = utils.create_network_with_sum(dim, latents_new, 
        #     n_layers = gen_layers, n_units = units, nonlinear = nn.ReLU)

        # ode_func_net = utils.create_network_with_sum(dim, latents_new, 
        #     n_layers = gen_layers, n_units = units, nonlinear = swish_layer)

        #nn.init.normal_(ode_func_net.weight, mean=0.0, std=1e-2)
    ...
    #     encoder_z0 = nn.ModuleList(layers)
        ...
        encoder_z0 = VAEEncoder(input_dim=enc_input_dim, hidden_dim=256, latent_dim=latents)
