...
def create_LatentODE_model(input_dim, z0_prior, obsrv_std, device, 
    ...
    if poisson:
        ...
        # ode_func_net = utils.create_network_with_sum(dim, latents, 
        #     n_layers = gen_layers, n_units = units, nonlinear = nn.Tanh)
    ...
    decoder_pose = Decoder(latents, angle_dim).to(device)
    ...
    diffeq_solver = DiffeqSolver(gen_data_dim, gen_ode_func, 'euler', latents, 
        odeint_rtol = 1e-3, odeint_atol = 1e-4, device = device)
    # diffeq_solver = DiffeqSolver(gen_data_dim, gen_ode_func, 'rk4', latents, 
    #    odeint_rtol = 1e-3, odeint_atol = 1e-4, device = device)
