...
class Dataset_base(Dataset):
    ...
    def sample_regular_extrap(self, images, poses, times, angle):
        ...
        # win_start = self.sample_size*random.randint(0, 8-1) + 1
        win_start = self.sample_size*random.randint(0, 16-1) + 1


        # print("Sample size: ", self.sample_size)
        # print("Win start: ", win_start)
        # win_start = 0
        # win_start = 100*random.randint(0,200/100-1)
        #win_start = random.randint(0, 100-self.sample_size)
        #win_start = 10*random.randint(0, 9)
        #self.sample_size = 100 - win_start

        input_images = np.concatenate((images[0:1], images[win_start: win_start + self.sample_size]), axis=0)
        poses = np.concatenate((poses[0:1], poses[win_start: win_start + self.sample_size]), axis=0)
        times = np.concatenate((times[0:1], times[win_start: win_start + self.sample_size]), axis=0)
        angle = np.concatenate((angle[0:1], angle[win_start: win_start + self.sample_size]), axis=0)

        # print(angle)
        mask = torch.ones((self.sample_size, 1))
        mask = mask.type(torch.FloatTensor).to(self.device)
        
        return input_images, poses, times, angle, mask, win_start/self.sample_size
