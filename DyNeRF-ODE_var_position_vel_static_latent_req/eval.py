...
def render_path_extrapolate(args, render_poses, rnn_model, render_times, hwf, latent_store, chunk, render_kwargs, gt_imgs=None, savedir=None,
                render_factor=0, save_also_gt=False, render_path_extrapolate=None, i_offset=0, latent_model=None, config=None):
    ...
    times = torch.arange(0,90)/89
    times = times[config["index"]:]
    times = times - times[0]
    time_obs = times[:1]
    ...
    c2w = render_poses[0]
    angle = config["pose"][:2]
    vel = [a - b for a, b in zip(config["pose_next"], config["pose"])]
    vel = vel[:2]
...
def train():
    ...
    if args.render_only or True:
        ...
        nerf_state_dict = torch.load('/hdd2/hiran/NeRF/DyNeRF-ODE_latent_dyn_var_position_vel_static_latent/logs_hill_near_new_vel_complete_seq_gt_pose_train_schedule/static_lat_200_2D_decoder_3layers_lips_prod_wt_1e-22_node_lr_1e-3_decay_0.999_on_cur_lr/pitcher_base/240000.tar')
        ...
        with open(os.path.join('/hdd2/hiran/NeRF/DyNeRF_ball_roll_diff_start_points_complete_seq_hill_near_new_static_frame/ground_truth_novel.json'), 'r') as fp:
            configs = json.load(fp)["test"]
        ...
        for i, config in enumerate(configs):
            ...
            with torch.no_grad():
                ...
                testsavedir = os.path.join(basedir, 'eval_novel_240k_ckpt', '{}'.format(str(i+1)))
                ...
                rgbs, _ = render_path_extrapolate(args, render_poses, vid_ode_model, render_times, hwf, latent_store, args.chunk, render_kwargs_test, gt_imgs=images,
                                    savedir=testsavedir, render_factor=args.render_factor, save_also_gt=False, latent_model=None, config=config)
