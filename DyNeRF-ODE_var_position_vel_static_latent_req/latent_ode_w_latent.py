...
class LatentODE(nn.Module):
    ...
    def get_reconstruction(self, time_steps_to_predict, truth, truth_time_steps, latent_truth_time_steps, vel=None,
        ...
        angle = torch.cat([angle, vel, latent_embeddings], dim=-1)

        sol_y = self.diffeq_solver(angle, time_steps_to_predict)
        
        sol_y_x = sol_y[...,-512:]
        sol_y_pose = sol_y[...,:63]
        sol_y_vel = sol_y[...,63:63+63]

        pred_x = torch.squeeze(self.decoder(sol_y_x))
        pred_pose = torch.squeeze(self.decoder_pose(sol_y_pose))
        pred_vel = torch.squeeze(self.decoder_vel(sol_y_vel))[:-1]
    ...
    def forward(self, batch_dict, warmup=False, freeze=False, div=False):
        ...
        latent_truth_time_steps = torch.tensor([[0]])
        ...
        return latents, seen, latents_pose, angle_latent_to_pred.squeeze(), None, latents_vel, vel_latent_to_pred.squeeze()
    ...
    def next_latent(self, latent, times_obs, times_to_pred, angle=None, loc=None, run_backwards=True, n_traj_samples=1, vel=0):
        ...
        if latent==None:
            ...
            vel = self.embed_vel(torch.tensor(vel)).squeeze()

        
        angle = angle.unsqueeze(0).unsqueeze(0)
        vel = vel.unsqueeze(0).unsqueeze(0)
        latents = latents.unsqueeze(0).unsqueeze(0)
        angle = torch.cat([angle, vel, latents], dim=-1)
