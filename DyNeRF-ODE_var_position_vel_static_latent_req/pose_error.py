...
def train():
    ...
    if args.dataset_type == 'blender':
        ...
        # dataloaders, times, render_poses, render_times, hwf, i_split, angle = load_blender_data(args, args.datadir, args.half_res, args.testskip, device)
        with open(os.path.join('/home/guests/hiran_sarkar/FullCode/datasets/DyNeRF_ball_roll_diff_start_points_complete_seq_hill_near_new_static_frame/ground_truth_novel_total_pose.json'), 'r') as fp:
            configs = json.load(fp)["test"]

        index = []
        angle = []
        for config in enumerate(configs):
            index.append(config["index"])
            angle.append(config["pose"][:,:2])



        # angle = angle[:-2]
        # angle = angle.reshape(-1, 90, 3)
        # angle = torch.tensor(angle)
        embed_angle, angle_dim = get_embedder(10, 2, 0)
        # gt_poses = embed_angle(angle)
    ...
    load_dict = torch.load('/home/guests/hiran_sarkar/FullCode/DyNeRF-ODE_latent_dyn_var_position/logs_hill_near_new_complete_seq/loc_mlp_latent_node_addnet_euler_seperate_pose_grad_align_80_ts_kessel0/pitcher_base/120000.tar')#["network_fine_state_dict"]
    ...
    for i, p in zip(index, pose):
        ...
        pose0 = pose[0]
        vel0 = pose[1] - pose[0]
    latents, pred_poses = latent_ode_model.next_latent(None,
                                            torch.tensor(time_obs),
                                            torch.tensor(time_to_pred), angle = pose0, vel=vel0)
    
    
    # pred_poses = pred_poses.reshape(-1, pred_poses.shape[1])
    # angle = angle.reshape(-1, pred_poses.shape[1])
    pred_poses = pred_poses.cpu().detach().numpy()
    print("Pred Poses: ", pred_poses[0,0])
    gt_poses = gt_poses.cpu().detach().numpy()
    print("GT Poses: ", gt_poses[0,0])
    exit()
    time_to_pred = time_to_pred.cpu().detach().numpy()

    errors = np.sum((pred_poses - gt_poses)**2, axis=-1)
    print("Errors: ", errors.shape)
    colors = ['#E63946', '#F1FAEE', '#A8DADC', '#F1A7A0', '#6C63FF', '#F9C74F', '#4D9DE0', '#FF6F61']
    for i, error in enumerate(errors):
        plt.scatter(time_to_pred,error, marker='o', color=colors[i])


    plt.title('POSE Error Plot')
    plt.xlabel('Time')
    plt.ylabel('Error')
    #plt.legend(handles=legend_elements, loc="best")
    plt.savefig('tsne_hill_near_new_complete_loc_mlp_latent_node_addnet_euler_grad_align_80_ts/pose_error.png')
