...
def render_path_extrapolate(args, render_poses, rnn_model, render_times, hwf, latent_store, chunk, render_kwargs, gt_imgs=None, savedir=None,
    ...
    # times = torch.arange(0,250)/49
    # time_obs = times[:1]
    # time_to_pred = times[::5]


    time = list(range(0,90))
    time = [i/89 for i in time]
    time_obs = torch.tensor(time[:1])
    time_to_pred = torch.tensor(time[:-29])
    # time_to_pred = torch.tensor(time[:-8])
    # render_pose = torch.tensor([[1.0, -5.82076470356796e-11, 4.06366089324324e-14, -0.018873000517487526], [5.82076470356796e-11, 1.0, 2.365361361766527e-24, 0.6559290289878845], [-4.06366089324324e-14, 0.0, 1.0, 2.1076200008392334], [0.0, 0.0, 0.0, 1.0]])
    render_pose = torch.tensor([[0.6616604328155518, -0.5762323141098022, 0.4797517955303192, 1.055518388748169], [0.7454885244369507, 0.5741128921508789, -0.33858722448349, -0.11665217578411102], [-0.08032678067684174, 0.5816792249679565, 0.8094423413276672, 1.6521778106689453], [0.0, 0.0, 0.0, 1.0]])
    ...
    #for j, frame_time in enumerate(render_times[3:]):
       ...
       ## Ball hill 2D GT
        # latent_nexts, _ = rnn_model.next_latent(None, torch.tensor(time_obs), torch.tensor(time_to_pred), 
        #                                     angle=[0.29651087522506714, 0.4808536171913147], 
        #                                     vel=[0., 0.])

        # latent_nexts, _ = rnn_model.next_latent(None, torch.tensor(time_obs), torch.tensor(time_to_pred), 
        #                                     angle=[0.29651087522506714, 0.4808536171913147], 
        #                                     vel=[1*(0.3145599067211151-0.29651087522506714), 1*(0.49201053380966187-0.4808536171913147)])

        # latent_nexts, _ = rnn_model.next_latent(None, torch.tensor(time_obs), torch.tensor(time_to_pred), 
        #                                     angle=[0.29651087522506714, 0.4808536171913147], 
        #                                     vel=[-(0.3145599067211151-0.29651087522506714), -(0.49201053380966187-0.4808536171913147)])

        
        ## Ball hill CNOS
        # latent_nexts, _ = rnn_model.next_latent(None, torch.tensor(time_obs), torch.tensor(time_to_pred), angle=[236/800, 489/800], 
        #                                         vel=[-2*(565-548)/800, -2*(475-483)/800])
        # latent_nexts, _ = rnn_model.next_latent(None, torch.tensor(time_obs), torch.tensor(time_to_pred), angle=[236/800, 489/800], 
        #                                         vel=[0, 0])

        # latent_nexts, _ = rnn_model.next_latent(None, torch.tensor(time_obs), torch.tensor(time_to_pred), angle=[548/800, 483/800], 
        #                                         vel=[(565-548)/800, (475-483)/800])
        # latent_nexts, _ = rnn_model.next_latent(None, torch.tensor(time_obs), torch.tensor(time_to_pred), angle=[548/800, 483/800], 
        #                                         vel=[0, 0])

        ## Ball Bowl
        # latent_nexts, _ = rnn_model.next_latent(None, torch.tensor(time_obs), torch.tensor(time_to_pred), angle=[356/800, 246/800], 
        #                                         vel=[-(358-356)/800, -(181-246)/800])

        # latent_nexts, _ = rnn_model.next_latent(None, torch.tensor(time_obs), torch.tensor(time_to_pred), angle=[356/800, 246/800], 
        #                                         vel=[0., 0.])

        # latent_nexts, _ = rnn_model.next_latent(None, torch.tensor(time_obs), torch.tensor(time_to_pred), angle=[358/800, 567/800], 
        #                                         vel=[0, 0])
        ...
        ## r_1538
        # latent_nexts, _ = rnn_model.next_latent(None, torch.tensor(time_obs), torch.tensor(time_to_pred), 
        #                                         angle=[-0.3076225817203522, 0.6599927544593811], 
        #                                         vel=[-0.4319654703140259-(-0.3076225817203522), 0.6703235507011414-(0.6599927544593811)])
        
        #1532
        # latent_nexts, _ = rnn_model.next_latent(None, torch.tensor(time_obs), torch.tensor(time_to_pred), 
        #                                         angle=[0.48490995168685913, 0.6672400236129761], 
        #                                         vel=[0.48490995168685913-(0.48490995168685913), 0.6672400236129761-(0.6672400236129761)])
        
        
        #1541
        # latent_nexts, _ = rnn_model.next_latent(None, torch.tensor(time_obs), torch.tensor(time_to_pred), 
        #                                         angle=[-0.5159640312194824, 0.6766805052757263], 
        #                                         vel=[-0.5062426924705505-(-0.5159640312194824), 0.6766805052757263-(0.6757769584655762)])

        #1536
        # latent_nexts, _ = rnn_model.next_latent(None, torch.tensor(time_obs), torch.tensor(time_to_pred), 
        #                                         angle=[0.11278848350048065, 0.6469878554344177], 
        #                                         vel=[-0.11825349926948547-(0.11278848350048065), 0.64900803565979-(0.6469878554344177)])


        # 1469
        latent_nexts, _ = rnn_model.next_latent(None, torch.tensor(time_obs), torch.tensor(time_to_pred), 
                                                angle=[-0.2639845013618469, 0.7053073048591614], 
                                                vel=[-0.28835198283195496-(-0.2639845013618469), 0.7065872550010681-(0.7053073048591614)])

        
        for j, latent_next in enumerate(latent_nexts):
        # for c2w, latent_next in zip(render_poses[:len(time_to_pred)], latent_nexts):
            print(j)
            c2w = render_pose
            ...
            if savedir is not None:
                rgb8_estim = to8b(rgbs[-1])
                filename = os.path.join(save_dir_estim, '{:03d}.png'.format(j+i_offset))
                imageio.imwrite(filename, rgb8_estim)
                if save_also_gt:
                    rgb8_gt = to8b(gt_imgs[i])
                    filename = os.path.join(save_dir_gt, '{:03d}.png'.format(j+i_offset))
                    imageio.imwrite(filename, rgb8_gt)
...
def train():
    ...
    if args.render_only or True:
        ...
        nerf_state_dict = torch.load('/hdd2/hiran/NeRF/DyNeRF-ODE_latent_dyn_var_position_vel_static_latent/logs_bowl_multi_cam/static_lat_200_2D_decoder_1layers_lips_prod_16_seqs_netwidth_512/pitcher_base/600000.tar')
        ...
        render_kwargs_test["network_fine"].load_state_dict(nerf_fine_state_dict)
        ...
        with torch.no_grad():
            ...
            testsavedir = os.path.join(basedir, expname, 'renderonly_{}_{:06d}_loc_pos_vel_ckpt_600k_1469'.format('test' if args.render_test else 'path', start))
