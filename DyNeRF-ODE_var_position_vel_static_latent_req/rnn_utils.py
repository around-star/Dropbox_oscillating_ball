...
def init_weight(size_out, size_in):
...
def init_weight_xavier_uniform(size_out, size_in):
    # Xavier uniform initialization
    limit = torch.sqrt(torch.tensor(6.0) / (size_in + size_out))
    return torch.empty(size_out, size_in).uniform_(-limit, limit)
...
#         # Xavier Uniform
#         # self.weight = nn.Parameter(init_weight_xavier_uniform(out_features, in_features))
...
#         self.bias = nn.Parameter(torch.empty(out_features).uniform_(-limit, limit))

#         self.bias = nn.Parameter(torch.zeros(out_features))
        
#         # Initialize `c` as the maximum row sum of the initialized weights
#         initial_c = torch.max(torch.sum(torch.abs(self.weight), dim=1, keepdim=True)) # matrix inf-norm
...
class LipschitzLinear(nn.Module):
    def __init__(self, in_features, out_features):
        super().__init__()
        # Use nn.Linear for defining the layer
        self.linear = nn.Linear(in_features, out_features)

        # Initialize `c` as the maximum row sum of the initialized weights
        with torch.no_grad():
            initial_c = torch.max(torch.sum(torch.abs(self.linear.weight), dim=1, keepdim=True))  # matrix infinity norm
        self.c = nn.Parameter(initial_c.view(1))

    def normalization(self, weight):
        abs_row_sum = torch.sum(torch.abs(weight), dim=1, keepdim=True)
        scale = torch.minimum(torch.tensor(1.0), F.softplus(self.c) / abs_row_sum)
        return weight * scale

    def forward(self, x):
        # Apply Lipschitz normalization to the weights
        normalized_weight = self.normalization(self.linear.weight)
...
def create_net_lipschtiz(n_inputs, n_outputs, n_layers = 1, 
    n_units = 100, nonlinear = nn.Tanh):
...
def create_network_with_sum(n_inputs, n_outputs, n_layers=1, n_units=100, nonlinear=nn.Tanh):

    # n_inputs = n_inputs+1
    
    # Create original network with activation
    original_network = create_net(n_inputs, n_outputs, n_layers, n_units, nonlinear)
    
    # Linear Network 1
    layers_without_activation = [nn.Linear(n_inputs, n_units)]
    # for i in range(1):
    #     layers_without_activation.append(nonlinear())
    #     layers_without_activation.append(nn.Linear(n_units, n_units))

    # layers_without_activation.append(nonlinear())
    layers_without_activation.append(nn.Linear(n_units, n_outputs))

    # Linear Network 2
    # kernel_size=4
    # layers_without_activation = [nn.Conv1d(in_channels=1, out_channels=1, kernel_size=kernel_size)]
    # for i in range(n_layers):
    # 	layers_without_activation.append(nn.Conv1d(in_channels=1, out_channels=1, kernel_size=kernel_size))
    # layers_without_activation.append(nn.Flatten())

    # final_size = n_inputs - (n_layers+1) * (kernel_size - 1)

    # layers_without_activation.append(nn.Linear(final_size, n_outputs))

    duplicate_network = nn.Sequential(*layers_without_activation)
    
    # Define a new network that sums outputs of both original and duplicate networks
    class NetworkWithSum(nn.Module):
        def __init__(self, original_network, duplicate_network):
            super(NetworkWithSum, self).__init__()
            self.original_network = original_network
            self.duplicate_network = duplicate_network

            #self._initialize_weights()
        
        def forward(self, x):
            output_original = self.original_network(x)
            output_duplicate = self.duplicate_network(x)
            return output_original + output_duplicate

        def _initialize_weights(self):
            for m in self.original_network:
                if isinstance(m, nn.Linear):
            ...
            for m in self.duplicate_network:
                if isinstance(m, nn.Linear):
                    nn.init.normal_(m.weight, mean=0.0, std=0.02)
                    if m.bias is not None:
                        # nn.init.constant_(m.bias, 0)
    ...
    return NetworkWithSum(original_network, duplicate_network)

def create_net_linear(n_inputs, n_outputs, n_layers = 1, 
    n_units = 100, nonlinear = nn.Tanh):
    layers = [nn.Linear(n_inputs, n_units)]
    for i in range(n_layers):
        layers.append(nn.Linear(n_units, n_units))
