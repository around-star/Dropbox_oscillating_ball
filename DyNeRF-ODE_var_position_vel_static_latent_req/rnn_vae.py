...
class RNN_VAE(nn.Module):
    def __init__(self, input_dim, latent_dim, rec_dims, 
        z0_prior, device, 
        concat_mask = False, obsrv_std = 0.01, 
        ...
        self.latent_net = LatentNetwork(input_size=num_frames)
        self.device = device
    ...
    def get_reconstruction(self, time_steps_to_predict, data, truth_time_steps, latent_truth_time_steps, 
        mask = None, n_traj_samples = 1, mode = None):

        #assert(mask is not None)
        latent_embeddings = self.latent_net(torch.squeeze(latent_truth_time_steps).int()).float()
        data = torch.unsqueeze(latent_embeddings, 0)
        ...
        return torch.squeeze(outputs), extra_info
    ...
    def forward(self, batch_dict):
        ...
        batch_dict["times"] = ((self.num_frames-1)*batch_dict["times"].to(self.device))


        latents, _ = self.get_reconstruction(
