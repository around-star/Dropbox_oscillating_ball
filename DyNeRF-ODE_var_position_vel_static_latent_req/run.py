...
def render_path_extrapolate(args, render_poses, rnn_model, render_times, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None,
    ...
    for i in range(1):
        ...
        # latent_nexts, _ = rnn_model.next_latent(None, times_obs, times_to_pred, angle=[-0.04509, 1.07723, 0.1000], vel = [0., 0., 0.])
        latent_nexts, _ = rnn_model.next_latent(None, times_obs, times_to_pred, angle=[-0.04509, 1.07723], vel = [0., 0.])        
...
def train():
    ...
    # nerf_state_dict = torch.load('/hdd2/hiran/NeRF/DyNeRF-ODE_latent_dyn_var_position_vel_static_latent/logs_bowl_multi_cam/static_lat_200_2D_decoder_1layers_lips_prod/pitcher_base/300000.tar')
    ...
    # loss_weights = torch.linspace(10, 0.1, steps=args.sample_size-1)
    for i in trange(start, N_iters):
        ...
        new_lrate_nerf = args.lrate * (decay_rate ** (global_step / decay_steps))

        # decay_rate_nODE = 0.999s
        # decay_steps = args.lrate_decay * 1000
        # new_lrate_nODE = max(args.lrate_node * (decay_rate_nODE ** (global_step / decay_steps)), args.lrate_node/10)
        # new_lrate_nODE = max(args.lrate_node * (decay_rate_nODE), args.lrate_node/10)
