...
class NeRFOriginal(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, latent_dim = 512, input_ch_time=1, output_ch=4, skips=[4],
        ...
        layers = [LipschitzLinear(input_ch+latent_dim, W)]
        for i in range(D - 1):
            if i in memory:
                raise NotImplementedError
            else:
                # layer = nn.Linear
                layer = LipschitzLinear
            ...
            layers += [LipschitzLinear(in_channels, W)]

        self.pts_linears = nn.ModuleList(layers)

        ### Implementation according to the official code release (https://github.com/bmild/nerf/blob/master/run_nerf_helpers.py#L104-L105)
        # self.views_linears = nn.ModuleList([nn.Linear(input_ch_views + W + latent_dim, W//2)])
        self.views_linears = nn.ModuleList([LipschitzLinear(input_ch_views + W + latent_dim, W//2)])
        ...
        if use_viewdirs:
            self.feature_linear = LipschitzLinear(W, W)
            self.alpha_linear = LipschitzLinear(W, 1)
            self.rgb_linear = LipschitzLinear(W//2, output_color_ch)
        else:
            self.output_linear = LipschitzLinear(W, output_ch)
