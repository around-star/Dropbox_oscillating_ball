...
def render_path_extrapolate(args, render_poses, rnn_model, render_times, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None,
    ...
    for i in range(1):
        ...
        print(i)
        print(times_to_pred)
        # latent_nexts, _ = rnn_model.next_latent(None, times_obs, times_to_pred, angle=[-0.03424198925495148, 0.6817569732666016, 0.5218473076820374])
        # latent_nexts, _ = rnn_model.next_latent(None, times_obs, times_to_pred, angle=[-0.04220002144575119, 0.5613999962806702, 0.5028529763221741], vel=[0., 0., 0.])
        latent_nexts, _ = rnn_model.next_latent(None, times_obs, times_to_pred, angle=[-0.04220002144575119, 0.5613999962806702], vel=[0., 0.])
        # latent_nexts, _ = rnn_model.next_latent(None, times_obs, times_to_pred, angle=[379/800, 427/800], vel=[0., 0.])
        # latent_nexts, _ = rnn_model.next_latent(None, times_obs, times_to_pred, angle=[-0.04220002144575119, 0.5613999962806702])
...
def create_nerf(args):
    ...
    ## For RNNODE
    # latent_ode_model = create_LatentODE_model(512, z0_prior, 0.01, device, latents=40, rec_dims=80, units=500, num_frames=args.num_frames)
    ...
    # Create optimizer
    optimizer_nerf = torch.optim.Adam(params=grad_vars_nerf, lr=args.lrate_nerf, betas=(0.9, 0.999), capturable=True)
    optimizer_node = torch.optim.Adam(params=grad_vars_node, lr=args.lrate_node, betas=(0.9, 0.999), capturable=True)

    # optimizer = torch.optim.Adam([
    # {'params': grad_vars_nerf, 'lr': args.lrate},
    # {'params': grad_vars_node, 'lr': args.lrate}], betas=(0.9, 0.999), capturable=True)
    ...
    return render_kwargs_train, render_kwargs_test, start, optimizer_nerf, optimizer_node, latent_ode_model
...
def lipschitz_loss_func(layers):
    """
    Computes the Lipschitz regularization loss based on the softplus of scaling parameters `c_i`.
    
    Args:
    - layers: A list of Lipschitz layers, each containing a learnable `c` parameter.
    
    Returns:
    - lip_loss: The Lipschitz regularization loss.
    """ 
    # lip_loss = 0.0
    lip_loss = 1.0
    for layer in layers:
...
def extract_lipschitz_layers(model):
    """
    Extract all LipschitzLinear layers from the given model.

    Args:
        model (torch.nn.Module): The neural network model.

    Returns:
        list: A list of all LipschitzLinear layers in the model.
    """
...
def extract_lipschitz_layers_decoder(model):
    """
    Extract all LipschitzLinear layers from the given model.
    ...
    # lipschitz_layers = [layer for layer in model if isinstance(layer, rnn_utils.LipschitzLinear)]
    lipschitz_layers = [layer for layer in model if isinstance(layer, encoder_decoder.LipschitzLinear)]
    return lipschitz_layers
...
def train():
    ...
    render_kwargs_train, render_kwargs_test, start, optimizer_nerf, optimizer_node, vid_ode_model = create_nerf(args)
    ...
    # nerf_state_dict = torch.load('/home/guests/hiran_sarkar/FullCode/DyNeRF-ODE_latent_dyn_var_position_vel_static_latent/logs_hill_near_new_vel_complete_seq_2_gt_pose/static_lat_200_no_concat_2D_decoder_3layers_lips_prod_wt_1e-22_5layers_nODE/pitcher_base/175000.tar')
    ...
    # nerf_scheduler = torch.optim.lr_scheduler.StepLR(optimizer_nerf, step_size=1000, gamma=0.1)
    # node_scheduler = torch.optim.lr_scheduler.StepLR(optimizer_node, step_size=1000, gamma=0.1)
    ...
    for i in trange(start, N_iters):
        ...
        #    div_loss = 0.00001*torch.mean((div_pred - div_target)**2)
            ...
            # grad_alignment = latents_grad_norm
            ...
            div_loss = 1e-3*wt_grad_alignment.sum(dim=0).abs()
        ...
        total_loss = angle_loss + loss + div_loss + vel_loss
        ...
        # if not latents_angle is None:
        #     ## Decoder
        #     lipschitz_layers = extract_lipschitz_layers_decoder(vid_ode_model.decoder.decoder)
        #     lipschitz_loss *= lipschitz_loss_func(lipschitz_layers)
        ...
        # if not latents_angle is None:
        # # # if False:
        #     lipschitz_layers = extract_lipschitz_layers(vid_ode_model.diffeq_solver.ode_func.gradient_net)
        #     lipschitz_loss = lipschitz_loss_func(lipschitz_layers)
        #     lipschitz_loss = 1e-7*lipschitz_loss
        ...
        l2_reg_loss = 0
        # if not latents_angle is None:
        #     l2_reg_loss = 0.1*torch.sum(latent**2)
        # total_loss += l2_reg_loss

        l2_nODE_reg_loss = 0
        # if not latents_angle is None:
        #     l2_nODE_reg_loss = 1e-6*sum(param.pow(2).sum() for param in vid_ode_model.diffeq_solver.ode_func.gradient_net.parameters())

        # total_loss += l2_nODE_reg_loss
        ...
        # nerf_scheduler.step()  # Update NeRF learning rate
        # if not latents_angle is None:
        #     node_scheduler.step()
        ...
        for param_i, param_group in enumerate(optimizer_nerf.param_groups):
            param_group['lr'] = new_lrate_nerf

        for param_i, param_group in enumerate(optimizer_node.param_groups):
        ...
        if i % args.i_print == 0:                
            ...
            if l2_reg_loss:
                tqdm_txt += " L2 Reg Loss: {:.6f}".format(l2_reg_loss.item())

            if l2_nODE_reg_loss:
                tqdm_txt += " L2 nODE Reg Loss: {:.6f}".format(l2_nODE_reg_loss.item())
