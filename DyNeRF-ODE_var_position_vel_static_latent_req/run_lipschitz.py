...
# from create_latent_ode_model_dit import create_LatentODE_model
...
def render_path_extrapolate(args, render_poses, rnn_model, render_times, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None,
    ...
    for i in range(1):
        ...
        # latent_nexts, _ = rnn_model.next_latent(None, times_obs, times_to_pred, angle=[358/800, 567/800], vel=[0., 0.])
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        # if i < 0:
        if i < 2000 or (i % 4000 >= 2000 and i % 4000 <= 2400):
        ...
        lipschitz_loss = 0
        # lipschitz_loss += lipschitz_loss_func(render_kwargs_train["network_fine"]._occ.pts_linears)
        # lipschitz_loss += lipschitz_loss_func(render_kwargs_train["network_fine"]._occ.views_linears)
        # lipschitz_loss += lipschitz_loss_func(render_kwargs_train["network_fn"]._occ.pts_linears)
        # lipschitz_loss += lipschitz_loss_func(render_kwargs_train["network_fn"]._occ.views_linears)
        # lipschitz_loss = 1e-6*lipschitz_loss
        ...
        # Rest is logging
        if i%args.i_weights==0 or i == 1999:
        ...
        if i % args.i_print == 0:                
            ...
            if lipschitz_loss:
                #tqdm_txt += f" Geom Mean Loss: {geom_mean_loss.item()}"
                tqdm_txt += " Lipschitz Loss: {:.6f}".format(lipschitz_loss.item())
        ...
        if (i%args.i_video==0 and i >= 10000) or i==200:
        #if True:
