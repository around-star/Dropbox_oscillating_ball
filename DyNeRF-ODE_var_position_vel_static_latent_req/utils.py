...
def split_data_extrap(data_dict, opt):
    ...
    start_obs = random.randint(1, int(data_dict["data"].shape[1])-n_observed_tp-65)
    # start_obs = random.randint(1, int(data_dict["data"].shape[1])-n_observed_tp-30)
    # start_obs = random.sample(list(range(0, int(data_dict["data"].shape[1])-n_observed_tp-40, 8)), 1)[0]

    # start_obs = 0

    split_dict = {"observed_data": torch.cat((data_dict["data"][:, 0:1].clone(), data_dict["data"][:, start_obs:start_obs+n_observed_tp].clone()), dim=1),
                # "observed_data": data_dict["data"][:, start_obs:start_obs+n_observed_tp].clone(),
                "observed_tp": torch.cat((data_dict["time_steps"][0:1].clone(), data_dict["time_steps"][start_obs:start_obs+n_observed_tp].clone()), dim=0),
                ...
                #   "data_to_predict": torch.cat((data_dict["data"][:, :start_obs].clone(), data_dict["data"][:, start_obs+n_observed_tp_pred:].clone()), dim=1),
                  "data_to_predict": data_dict["data"][:, start_obs+n_observed_tp_pred:].clone(),
                #   "tp_to_predict": torch.cat((data_dict["time_steps"][:start_obs].clone(), data_dict["time_steps"][start_obs+n_observed_tp_pred:].clone()), dim=0),
                  "tp_to_predict": data_dict["time_steps"][start_obs+n_observed_tp_pred:].clone(),
    ...
    if ("poses" in data_dict) and (data_dict["poses"] is not None):
        split_dict["poses"] = torch.cat((data_dict["poses"][:, 0:1].clone(), data_dict["poses"][:, start_obs:start_obs+n_observed_tp].clone()), dim=1)
        ...
        split_dict["poses_to_pred"] = data_dict["poses"][:, start_obs+n_observed_tp_pred:].clone()
    
    if ("times" in data_dict) and (data_dict["times"] is not None):
        split_dict["times"] = torch.cat((data_dict["times"][:, 0:1].clone(), data_dict["times"][:, start_obs:start_obs+n_observed_tp].clone()), dim=1)
        ...
        split_dict["times_to_pred"] = data_dict["times"][:, start_obs+n_observed_tp_pred:].clone()
        split_dict["times_all"] = data_dict["times"].clone()

    if ("angle" in data_dict) and (data_dict["angle"] is not None): 
        split_dict["angle"] = torch.cat((data_dict["angle"][:, 0:1].clone(), data_dict["angle"][:, start_obs:start_obs+n_observed_tp].clone()), dim=1)
        ...
        split_dict["angle_to_pred"] = data_dict["angle"][:, start_obs+n_observed_tp_pred:].clone()
